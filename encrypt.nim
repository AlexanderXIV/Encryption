...
# proc `$`(data: Block): string =
#   for y in 0..<4:
#     for x in 0..<4:
#       result &= toHex(data[4 * x + y]).toLower & " "
#     result &= "\n"
...
proc mul(n1, n2: uint8): uint8 =
  if n2 == 1:
    ...
    return n1.mul2() xor n1

proc sub_bytes(data: var Block) =
  for i, item in data:
    data[i] = save_box[item]
...
proc mix_columns(data: Block): Block =
  var x, y: int
  for i in 0..<16:
    result[i] = mul(data[y], matrix[x]) xor mul(data[y + 1], matrix[x + 1]) xor mul(data[y + 2], matrix[x + 2]) xor mul(data[y + 3], matrix[x + 3])
    if x == 12:
      x = 0
      y += 4
    else:
...
proc encrypt*(data, key: Block): Block =
  ...
  var round_key = key
  for i in 0..<rounds:
    round_key = makeRoundKey(round_key, i)
    result.sub_bytes()
    result.shift_rows()
    result = mix_columns(result)
    result.add_round_key(round_key)

  round_key = makeRoundKey(round_key, rounds)
  result.sub_bytes()
  result.shift_rows()
  result.add_round_key(round_key)